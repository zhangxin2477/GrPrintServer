 ...
 */
public class BaseSlidingMenu extends RelativeLayout {
    ...
    public BaseSlidingMenu(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setCenterView(View center) {
        ...
        //bgShadeContent.setBackgroundDrawable(getResources().getDrawable(R.mipmap.shade_bg));
    ...
    public void computeScroll() {
        if (!mScroller.isFinished()) {
            if (mScroller.computeScrollOffset()) {
                int oldX = centerView.getScrollX();
                int oldY = centerView.getScrollY();
                int x = mScroller.getCurrX();
                int y = mScroller.getCurrY();
                if (oldX != x || oldY != y) {
                    if (centerView != null) {
                        centerView.scrollTo(x, y);
                        if (x < 0)
    ...
    private boolean canSlideRight = false;//false;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (xDiff > mTouchSlop && xDiff > yDiff) {
                    if (canSlideLeft) {
                        float oldScrollX = centerView.getScrollX();
                        if (oldScrollX < 0) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                mLastMotionX = x;
                mLastMotionY = y;
                if (centerView.getScrollX() == -getMenuViewWidth()
                        && mLastMotionX < getMenuViewWidth()) {
                    return false;
                }

                if (centerView.getScrollX() == getDetailViewWidth()
                        && mLastMotionX > getMenuViewWidth()) {
                    return false;
                }
            ...
            case MotionEvent.ACTION_MOVE:
                if (mIsBeingDragged) {
                    ...
                    if (centerView != null) {
                        centerView.scrollTo((int) scrollX,
                                centerView.getScrollY());
                        if (scrollX < 0)
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    int oldScrollX = centerView.getScrollX();
                    int dx = 0;
                    ...
                    if (oldScrollX >= 0 && canSlideRight) {//right view
                        if (xVelocity < -VELOCITY) {
    ...
    void smoothScrollTo(int dx) {
        int duration = 500;
        int oldScrollX = centerView.getScrollX();
        mScroller.startScroll(oldScrollX, centerView.getScrollY(), dx,
                centerView.getScrollY(), duration);
        invalidate();
    }
